...
See the github description for a more comprehensive write-up.  This was one of those annoying and intermittent bugs, and it would cause guards to flicker/disappear as you travelled through the city.  This has hopefully been solved, but by the nature of these bugs it's hard to know for sure this early on.
...
This resolved many feature gaps in Jak 2 graphics-wise. Generic was needed for a variety of things, while warp is used for distortions such as the warpgates or the mirror in the Hip Hog.
...
With the generic renderer being added, the orb and skullgem in the HUD can now be drawn properly. The skullgem is currently not moving because it still requires texture animations which we have not started work on yet.

![](./img/foreground-hud.png)

### HUD Related Side-mission Crash <PRLink href="https://github.com/open-goal/jak-project/pull/2540"/>

Fixes a crash that occurred on the PS3 release.  It happens because when you have access to the `city-krew-collection-introduction` mission, it loads the `lsack` level no matter where you are in the city.  This causes the `lbush` level to be removed too fast when you complete the side mission, causing the crash.
...
This may not be fixed in the optimal way yet, but there was an issue in Drill Platform where the `excitement` would always be maxed.  This was traced back to the `ginsu`'s bones being `NaN`
...
For the longest time, we have been missing this functionality which has been the cause of dozens of bugs, but thanks to Ziemas' and water111's hard work at reverse engineering Overlord, we finally have full cutscene audio and voice lines in
OpenGOAL!
...
The minimap would be inappropriately stretched based on the aspect ratio.
...
- The compilation speed is quite slow in comparison, but trying to be improved.
- Documentation for the new format needs to be written

But this should hopefully lead to a better understanding of the particle definitions and make them easy to use.

### More efficient divisions <PRLink href="https://github.com/open-goal/jak-project/pull/2585"/>

Some slight modifications to division were done to make them more efficient.
