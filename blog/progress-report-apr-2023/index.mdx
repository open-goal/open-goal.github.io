...
The Japanese release does have a bunch of regional changes though, such as removing Jak's goatee and not allowing you to kill civilans in the city.
...
Fixes a crash that also occurred on the PS3 release.  It happens because when you have access to the `city-krew-collection-introduction` mission, it loads the `lsack` level no matter where you are in the city.  This causes the `lbush` level to be removed too fast when you complete the side mission, causing the crash.
...
This may not be fixed in the most optimal way yet, but there was an issue in Drill Platform where the `excitement` would always be maxed.  This was traced back to the `ginsu`'s bones being `NaN`
...
OpenGOAL!  This change alone resolved between 30-40 issues we've been tracking.
