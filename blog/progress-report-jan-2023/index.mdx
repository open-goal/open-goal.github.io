...
The first progress report to kick off the year, perhaps surprisingly, it's going to be a big one!  Lots of missions are partially working and large portions of the remaining complicated systems in Jak 2 are on their way to completion.  We even have some new features in Jak 1 to share!
...
Probably the biggest thing that happened this month was the alpha release of Kuitar's custom level, **Crystal Cave**. Despite custom level tooling currently hit-or-miss, it is amazing what Kuitar has already achieved.  Having such a high-fidelity example hopefully inspires others to follow suit and continues to help motivate development for better tooling!  You can check it out via the [Unofficial Mod Launcher](https://opengoal-unofficial-mods.github.io/)
...
<ReactPlayer controls url={require("./video/il-select.mp4").default} />
...
<ReactPlayer controls url={require("./video/checkpoint-select.mp4").default} />
...
The core code for the titansuit / mechsuit was decompiled, and for the most part works in the relevant missions.
...
At first we were just concerned with fixing the issue related to the ocean lacking collision.  But now the ocean is more or less fully rendering with only some minor issues related to the transition between the `near` and `mid/far` ocean sections.
...
<ReactPlayer controls url={"https://www.youtube.com/watch?v=f6HPWKO0yAc"} />
...
<ReactPlayer controls url={"https://www.youtube.com/watch?v=e3IBm6SwAyI"} />
...
Not merged yet, but ambient support is being added for custom levels.  This lets you do things like play hints when Jak enters a given area.
