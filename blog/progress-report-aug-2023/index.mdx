...
After we started to work on Jak 2, we split up our file paths so we have `jak1` and `jak2` folders. However, some of the debug code which reads compiled objects files at runtime was never changed to accomodate this until now.
...
OpenGOAL now supports Jak 2's fancy Japanese subtitles, the splash screens and the scrap books.
...
A small issue, but fixed nonetheless. One easy way to trigger it would be entering an airlock with the JET-Board and observe the JET-Board theme not playing on the new level you just entered.
