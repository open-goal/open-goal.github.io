...
The minimap is now mostly functional! There are a few bugs with some icons stretching across the entire minimap and the minimap looking low-res, which will be fixed at a later point.
...
A fairly significant performance issue was found involving File I/O and memory card file saving. If you are using a weaker PC or slower hardware (HDD instead of an SSD) you may notice a signifcant difference.
...
When the bombbots exploded, the game would crash. This has now been fixed as well, which means the bombbots in missions now work, including side missions that reuse this logic.

<!-- TODO - bombbots missions -->

### Texture Scrolling <PRLink href="https://github.com/open-goal/jak-project/pull/2303"/>

This feature is also used in Jak 1 now, but it is more impactful in Jak 2 where a decent amount of assets were dependent on it, namely most conveyor belts found throughout the game as well as the rim on warp gates. In Jak 1 this means a reduced dependence on `generic` which translates to improved performance.
...
There were a ton of texture related problems, most notably related to the lasers being red instead of blue due to sharing the same image but different palettes. This has been fixed.
...
Fix some bugs with the decompilation of one of the functions for drawing actors on-screen causing elements to appear solid black. Notably the turrets in the palace or certain airlocks.
...
Many particle groups (effects) had the wrong parameters for scale and rotation due to a bug in the decompiler, including but not limited to: the flame pots in Mar's Tomb, panels in the Fortress leading up to the prison cells, the dark eco pickup, signs scattered all over the city, fruit from baskets and stands, the warp gate and the Rift Ring. These now render as they should.

<!-- TODO pics -->

### Fog fixes <PRLink href="https://github.com/open-goal/jak-project/pull/2424"/>

`disable-fog` is handled better now, for example the searchlights on the fortress now appear much better than before, though some other things are temporarily broken for now.

<!-- TODO pics -->

### `enter-state` and HUD related sprite crash <PRLink href="https://github.com/open-goal/jak-project/pull/2428"/>

`HUD` related sprite assert issues have been fixed, which were causing some crashes in some cutscenes and during rain. Also, the underlying issue for why Sig could never jump up the first pipe in his escort mission has been fixed. It was due to a missed condition added between Jak 1 and Jak 2.
...
A few nasty bugs from the original game that were causing crashes have been fixed, such as loading the palace cable mission after spawning in the city which was causing one of the level files to start loading and then immediately get cancelled, which is not handled properly. Another bug that was causing crashes in both the original PS2 game and OpenGOAL was actors placed around levels using types from different levels that could be unloaded. The easiest way to trigger this one is by going out of bounds at the end of the "Rescue Friends in Fortress" mission. Now you can go out of bounds to your heart's content!
