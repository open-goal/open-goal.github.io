...
This was a very big change but it did go through a lot of testing so hopefully no major issues are found. However if you do find any major issues, report them as per usual!
...
This is a Windows only change.  When pressing the screenshot keybind the image will also be placed on your clipboard so you can paste them somewhere like Discord.

Some other improvements around screenshots were mergeed as well.
...
Fixes problems around ambush and battle sections. For example all of the `monster-frog`s in Mar's Tomb would immediately spawn at one location in the lobby area.
...
Some additional work was done on the remaining code for the in-game editors. The next main task will be seeding the database with all of the game's data so the editor's actually have something to edit.
