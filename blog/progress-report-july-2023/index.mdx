...
There may be some issues as this is the first macOS release, but you *should* be able to install the game via the launcher just like any other supported platform. The [installation instructions](/docs/usage/installation/) have been updated, however there isn't a whole lot to say -- the launcher installs like any other common MacOS application.  Download the `.dmg` and drag-and-drop it into your `Applications` folder.
