...
No. Pressure sensitivity is the possibility of a few controllers such as DualShock 2, DualShock 3 to detect differences in the amount of force applied to them, as opposed to binary inputs (e.g. "pressed" or "not pressed"). Even though this seems appealing for driving vehicles, there is no plan to support this for face buttons. The vast majority of controllers doesn't support this at a hardware or driver level anyway. In the future, we possibly support an option to use triggers to accelerate combined with pressure sensitivity.
