...
function makeTableItemBool(val, note_id = -1) {
  ...
  if (val === "partial") return <td class="lang-maybe">Partial{notes}</td>;
  else if (val === "invalid") return <td>N/A{notes}</td>;
  else if (val === true) return <td class="lang-yes">Yes{notes}</td>;
  else return <td class="lang-no">No{notes}</td>;
}
