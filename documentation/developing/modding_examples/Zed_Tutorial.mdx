---
sidebar_position: 2
title: Zed Starting Tutorial
description: A deep dive into the code changes for the Checkpoint Randomizer Mod
---

# Getting Started making mods

<PostAuthors authors={["ZedB0t"]} authorNames={["ZedB0T"]} />

- Introduction
- Getting Required/Recommended Software
  - Github Desktop
  - Visual Studio code
    - OpenGOAL extention
  - Visual Studio
  - Scoop
  - Opening a proper terminal in directory
- Github
  - Creating a Org
    - Forking mod base
    - looking at changes
  ...
  - Introduction to Binaries
    - Goalc
      - LT/MI/ML
      - Forward Declare
      - Add Custom Goal Code file to compile list
    - Extractor
      - Compile time / Extraction time
    - gk
      - Just the game lol
  - Goal/CPP/Texture mods
    - GOAL
      - What GOAL does/is
        - Syntax/Example
      - CPP
        - Changes that require CPP changes
        - Cover rebuild the binaries here
      - Extraction changes
        - Cover FR3/task -extract stuff here
        - Mention what is stored in the fr3, and why it can only be done on extract etc
- Practice
  - Processes
    - What is it? How to create one
  - Mod that does something when you get a orb
  - Mod that does something on a defined random chance (Different each time)
  - Texture replacement mod
  - Custom level mod

# Beginner Modding tutorial

## Introduction

 <div style={{ textAlign: 'center' }}>
       ![](./img/zed_tutorial/mainimg.png)
    </div>
TODO  

👋 Welcome to this beginner modding tutorial! In this walkthrough, we'll guide you through the process of creating a basic mod for OpenGOAL. This tutorial covers essential components, such as modifying GOAL code, making C++ changes, and replacing textures and building a custom level. Let's dive into the basics of modding and explore the exciting world of game customization.

:::danger
This guide is written in a way designed to be easy for everyone to understand/follow regardless of experiance. Because of this some terminology and practices might not be 100% optimal or correct, but hopefully it makes it easier to start your learning journey. 
:::

### Getting Required/Recommended Software

1. **Github Desktop**
   - Version control platform with a user-friendly interface.  
   [Download here](https://desktop.github.com/)

2. **Visual Studio Code**
   - Lightweight code editor with extensibility.  
   [Download here](https://desktop.github.com/)
   - *OpenGOAL Extension:* 
     - Enhances support for OpenGOAL development.
     - Automatically reload changes into game on file save.
     - Color code the code to make it way easier to read/work with.

3. **Visual Studio**
   - Comprehensive integrated development environment (IDE).  
   [Download here](https://desktop.github.com/)
   - Used to rebuild the CPP binaries - Goal code modification should be done in `Visual Studio Code` with the extention

4. **Opening a Proper Terminal in Directory**
   - Ensure easy navigation and execution of commands in the desired directory.  
   <iframe
  width="640"
  height="360"
  src="https://www.youtube.com/embed/HjLXH45t-o0?si=AULlbVIKFfV8xdzI"
...
5. **Scoop**
   - Get Scoop from [here](https://scoop.sh/) by running the following two lines in your terminal window
   ```opengoal
    Set-ExecutionPolicy RemoteSigned -Scope CurrentUser
    irm get.scoop.sh | iex
    ```
   - Close and reopen your terminal then run to get all the software you need.
   ```opengoal
   scoop install git llvm nasm python task
   ```
:::danger
If you run into issues run 
`scoop update *` in your terminal to update all of the scoop tools.
:::
...
Learning github is probably one of the harder and more frustrating things about learning how to make a OpenGOAL mod. Please understand that it is weird complex and frustrating and will take effort to understand/work with. It is normal to struggle and you should not get discouraged, hopefully the following guide makes it a bit easier to understand.

### What is Git?

Github is a way to Post your mod and changes online, so people can see the changes and download your mod and or help you. You can do github stuff in the command line terminal, but for the purposes of this guide, we will be using github desktop. You will need to create a github account [here](https://github.com/).
   - Creating a organization
     - Creating a organization is free, and you can kind of think of it as a kind of "folder" to keep all of your mods in.
     - After Signing into your github account, you can create a organization [here](https://github.com/account/organizations/new?plan=free&ref_cta=Create%2520a%2520free%2520organization&ref_loc=cards&ref_page=%2Forganizations%2Fplan)
   - Forking Mod Base
     - We have created a modified version of openGOAL that is designed with modding in mind. This version features a few quality of life changes to make modifiing and running your custom code easier.
     - You can create your own mod or "fork" of this version by visiting [here](https://github.com/OpenGOAL-Mods/OG-Mod-Base/fork)
   - Downloading your mod
     - ok We have created a github page for your mod, but how do we actually download it and make changes?
   - Examining Changes
     - all the mods on the modlauncher are listed at [jakmods.dev](https://jakmods.dev/). You can visit this site, and click the website button for each mod to view its github page and study the changes that were made to make them work.
   - Branches
     - What is a branch?
       - A branch is basically a copy of your mod where you can make and upload your changes, then you can combine/merge/push these changes back to your "main" branch. You can create a branch by clicking the current branch, then new branch button in github desktop TODO add images
     - Making Changes
       - Once you have made a branch to work on, click on the repository tab in github desktop, then click Open in visual studio code. This will open your files so you can start making your changes.
       - After you have made some changes to some files be sure to test and make sure they work and compile, then you can describe the change in the Summary field, and commit the change to your development branch.
     - Pushing Changes
       - Once you have all your changes and want to bring the changes from your development branch to your main branch, click the Create pull request button and select your forks main branch.
     - Merging
        - After that you can merge the pull request or assign it to someone else to review and merge if you are working together.
     - Collaborating

### Introduction to Binaries
   - GOALC
     - GOALC.exe is the compiler for the game. It can be opened by running `task repl` in your terminal.
     - Commands
       - LT/MI/ML
         - (mi)
           - This command builds the game and all of your GOAL changes. If you make GOAL changes, you will need to run this to add them to your mod.
         - (lt)
           - This command is used to connect goalc to the currently running game, this lets you run/test code and see values in real time
           - TODO add a video about lt/mi
         - (ml) `L`
           - This command hot reloads one file while the game is running, this is useful for seeing your changes in real time and is automatically done by the visual studio code OpenGOAL extention.
           - Keep in mind that in order to actually permanetly add the changes to the mod, you will still need to run `(mi)`
     - Forward Declare/reload
       - In order to use a function/variable/type that is defined in a later function we would need to Forward Declare it so the compilier knows what type it will be when it is declared.
        see more in  
        TODO link to syntax
     - Adding Custom Goal Code File to Compile List
       - When possible it is prefered to add custom GOAL code to new files or PC exclusive ones. Sometimes the decompiler can be improved, and this results in the outputted original code changing etc. This can cause conflicts if you have modified original code.  
       TODO link to game.gp
   - Extractor
     - Extractor can be ran with `task extract` in your terminal
     - When you run this it builds all the files that `gk` needs in order to display the game on screen. 
     - Since this is where it builds the graphical files, texture replacements and stuff must rerun this to see the changes. A `(mi)` does not rebuild the graphical files and will not show texture mod changes.
   - gk
     - This is the openGOAL program/window.
     - You can open this with the command `task boot-game` in your terminal
     - If you want to add CPP libraries to your mod, you would need to add the libraries here and rebuild the binary (networking support/audio support/RGB support etc.)

### OpenGOAL File/Folder structure
TODO
...
### Goal/CPP/Texture Mods
   - GOAL
     - What GOAL Does/Is
       - #### Syntax/Examples
         - Preforming Basic logic/math  
         GOAL is a LISP Dialect so It uses parens `()` to denote function/macro calls  
         If we want to preform some basic operation, we would denote it as follows
         ```opengoal
         (+ 1.0 3.0)
         ;; in this case, the + operator is used to get the sum of argument 1 (1.0) 
         ;; and argument 2 (3.0) This call outputs the result and that would be 4.0
         ```
         - Variables  
         We can use Variables to store and reference specific values, in the following example we repeat the above example, however, we use variables to make the code easier to read.
        ```opengoal
        ;;define variables to use in the function.
        (define *argument-1* 1.0)
        (define *argument-2* 3.0)
        (define *addition-result* 0.0)

        ;;set the value of *addition-result* to the value of (+ *argument-1* *argument-2*)
        (set! *addition-result* (+ *argument-1* *argument-2*))

        ```
        - Conditional Statements
          - GOAL truthy  
          :::danger Everything in GOAL that is not `#f` is considered as "true" 
          This can be 0, function calls, or variables. If it is not explicitly `#f` then it is considered true.
          :::
          - If statements  
            An `if` statment takes in 3 arguments, the first argument is the condition, the second argument is the code that is executed if the condition is true and the third argument is the code that is executed when the condition is false.
            ```opengoal
            (if (= (-> *game-info* money) 7.0)
            (format #t "We have 7 orbs")
            (format #t "We do not have 7 orbs")
            )

            ;; (= (-> *game-info* money) 7.0) compares the value of where the game stores 
            ;; our orb count (-> *game-info* money) to 7.0, and returns #t if it equal 
            ;; or #f if it is not equal
            ```
        - Functions
          - Used to organize code and preform standard operations
          - Functions are always wrapped in parens `()`
          - Functions return the last value in the function. This is the reason so many functions end in `#f` or `(none)`
            - Example functions
             - Add one orb to jaks orbs when called 
             ```opengoal
             (defun increase-orbs-by-one ()
             (set! (-> *game-info* money) (+ (-> *game-info* money) 1.0))
             (none))
             ```
             - Add a specified number of orbs to jak
             ```opengoal
             (defun increase-orbs-by-number ((add-amount float))
             (set! (-> *game-info* money) (+ (-> *game-info* money) add-amount))
             (none))
             ```
               - You would call this function by running `(increase-orbs-by-numer 32.0)`
             - Check if Jak has at least 13 powercells when called and return true or false
            ```opengoal
            (defun have-13-or-more-cells? ()
              (if (=> (-> *game-info* fuel) 13.0)
                #t
                #f
              )
            )
            ```
        - Hooks  
           Now that we have created a function, we need to run the code in the game.  
           One way to do this is by creating a "hook" function
           - The mod base already includes a few basic hooks so lets utilize those for now.
           ```opengoal
           ...
           ```  
           We can add a call to our function here by adding `(increase-orbs-by-numer 2.0)` to the body. This function is already inserted into the original games code in the area that runs when you pick up a scout fly, so any code or functions we add to this hooks body will automatically be ran when a fly is collected. This means that now when we pickup a scout fly, we also expect that our orbs should increase by 2.
     - CPP
       - Changes Requiring CPP
       - Cover Rebuilding the Binaries Here
     - Extraction Changes
       - Cover FR3/Task -Extract Stuff Here
       - Contents of FR3 and Why Extraction is Necessary
...
### Mod Examples
   - Mod That Reacts to Getting an Orb
   - Mod That Triggers on a Defined Random Chance
   - Texture Replacement Mod
   - Custom Level Mod

## Conclusion

🚀 This walkthrough aims to provide a comprehensive guide to building OpenGOAL mods, using the Checkpoint Randomizer mod as a case study. Let's dive in!


## Background

👋 Today I'm going to walk through the changes we made to build the Checkpoint Randomizer mod. To my knowledge, this was one of the earliest OpenGOAL mods, and as such there was a lot of trial and error to get things right. I'll do my best to describe the _process_ of figuring out how things worked and what we needed to do, in addition to walking through the latest version of the code.
...
<iframe
  width="640"
  height="360"
  src="https://www.youtube.com/embed/MPPQ-9oYs3c?start=1013"
  title="YouTube video player"
  frameborder="0"
...
#### What happens when you touch a Power Cell?
...
#### Disabling Power Cell cutscenes
...
#### Debug menu - Actor Marks

To unlock the Citadel door, first we need to figure out its type so that we can identify the code that's involved. In the debug menu, there's a useful setting we can toggle to help with this: `Actor > Actor Marks > On`.
...
#### Removing invisible wall (`blocking-plane`)
...
#### Displaying collectable/warp counter
...
<iframe
  width="640"
  height="360"
  src="https://www.youtube.com/embed/B5mEhVt0v68"
  title="YouTube video player"
  frameborder="0"
  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
  allowfullscreen
...
- [`progress-draw.gc`](https://github.com/open-goal/jak-project/blob/6d99f1bfc1aa4f3ab5a5f3dfca7d4688434f2984/goal_src/jak1/engine/ui/progress/progress-draw.gc)
  ...
  - the _original_ Options menu would be drawn here as well
...
- [`progress-static.gc`](https://github.com/open-goal/jak-project/blob/6d99f1bfc1aa4f3ab5a5f3dfca7d4688434f2984/goal_src/jak1/engine/ui/progress/progress-static.gc)
  - the _original_ Options menu is defined here
...
- [`progress.gc`](https://github.com/open-goal/jak-project/blob/6d99f1bfc1aa4f3ab5a5f3dfca7d4688434f2984/goal_src/jak1/engine/ui/progress/progress.gc)
  - the _original_ logic for transitioning through the menus, responding to controller inputs, etc is here
...
- [OpenGOAL Mod Base - Github](https://opengoal-unofficial-mods.github.io/modbaseRedirect.html)
