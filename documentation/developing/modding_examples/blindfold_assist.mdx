...
For centering the camera behind Jak, we will also need to run some code every frame in order to check if the user has pressed our desired button combo. Lucky for you, we've already created a helper function `runs-every-frame` [in the opengoal-mod-base](https://github.com/OpenGOAL-Mods/OG-Mod-Base/blob/main/goal_src/jak1/engine/mods/mod-custom-code.gc#L24-L28) that gets executed on every frame of the [main display loop](https://github.com/OpenGOAL-Mods/OG-Mod-Base/blob/main/goal_src/jak1/engine/game/main.gc#595)!
