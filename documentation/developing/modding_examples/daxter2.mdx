...
I originally wanted to get this concept working for the JET Board, and so I updated the condition to use a `case` to filter out the `target-board` states:
...
Well, I decided to give it a shot and try the same approach for ground pounds. First of all, we'll need to add code to allow `target` to transition from states like `indax-jump` into a ground pound. We can probably borrow some code from where the non-`indax` jump state(s) transition into a ground pound. To verify the name of the state(s) we're interested in, we can use the debug menu - if we enable `Target` > `Target Stats`, a bunch of information is shown on screen, including Jak's current state.

With this, we can see Jak's ground pound is the `target-flop` state, and we can search through the code to see how `target` gets there. [This snippet of code](https://github.com/open-goal/jak-project/blob/897e9c588b344bd86838e9ea0ef2ef642130069a/goal_src/jak2/engine/target/target.gc#L1116-L1138) checks for the square button press and sends Jak into a ground pound from the `target-jump` state under certain conditions. Similar code can be found in the `target-double-jump`, `target-high-jump`, and `target-attack-uppercut-jump` states (with different parameters), but for Daxter's version I decided to just reuse the code from `target-jump` within `target-indax-jump` and `target-indax-double-jump`:
...
Before I could actually test anything though, I'd also need to add Jak's ground pound states to the allowlist I started with the JET Board states. Without this, you would be able to *initiate* a ground pound as Daxter, but you'd be immediately forced back into the `indax` mode, cancelling the ground pound. From some trial and error, I decided to allow these few states to cover the start of the ground pound through to any bounce/jump out of it:
...
The animations were a little abrupt when going between `indax` mode and ground pounds, and the momentum felt a little unnatural... But overall it was good enough for me at this stage in the project, I was mostly just happy to give the player a way to break the grates and unblock that part of the first mission.
