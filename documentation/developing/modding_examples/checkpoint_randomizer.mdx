...
There are a handful of methods that needed to be implemented, but they're all pretty straightforward and just needed to be tailored to the `randomizer-settings` fields:
...
- [`progress-part.gc`](https://github.com/open-goal/jak-project/blob/6d99f1bfc1aa4f3ab5a5f3dfca7d4688434f2984/goal_src/jak1/engine/ui/progress/progress-part.gc)gametextenprogre
...
Let's take a look at the root Options menu as a starting point:
...
`*progress-carousell*` refers to the type of menu option where you cycle left/right between various options, often used when there are 3+ choices. The way this style of menu option operates is a bit strange, but in short the `int-backup` field is used to [store or reset to your last selection in the `respond-common` method](https://github.com/open-goal/jak-project/blob/6d99f1bfc1aa4f3ab5a5f3dfca7d4688434f2984/goal_src/jak1/pc/progress-pc.gc#L1492-L1695).



draw-notice-screen - (draw-options self
