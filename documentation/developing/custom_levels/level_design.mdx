...
- **Linear loops**. They are quite similar to the previous one. The big difference is that the level doesn't (normally) let you go backward to it. They have the same similar small diverging paths to break the linearity but the general level does loop in one direction back toward the start. _Examples of these: Geyser Rock, Lost Precursor City, Boggy Swap._
...
- **Connector levels**. As these are there as a link between two hub levels, they're almost strictly linear levels from point A to point B with very little in the way of diverging path except for shortcuts. _Examples of these: Fire Canyon, Mountain Pass, Lava Tube._
