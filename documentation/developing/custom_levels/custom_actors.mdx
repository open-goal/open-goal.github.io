...
This tool supports Jak 1, Jak 2 and Jak 3. This guide will only cover Jak 1, but the overall process is the same, the main differences will be in the GOAL code setup.
...
Add an entry for `test-actor-ag.go`. This is your actor's art group file which is automatically generated by the Build Actor tool. Also add an entry for the GOAL code file that will contain your actor logic, in this case `test-zone-obs.o`.
...
The guide also focuses on Jak 1. The steps for Jak 2 and 3 will be very similar, but vary in some spots. Check the respective `test-zone-obs.gc` file for each game for examples:

- [Jak 1](https://github.com/open-goal/jak-project/blob/master/goal_src/jak1/levels/test-zone/test-zone-obs.gc)
- [Jak 2](https://github.com/open-goal/jak-project/blob/master/goal_src/jak2/levels/test-zone/test-zone-obs.gc)
- [Jak 3](https://github.com/open-goal/jak-project/blob/master/goal_src/jak3/levels/test-zone/test-zone-obs.gc)
...
Our object needs a `skeleton-group` to function. The vanilla code uses the `defskelgroup` macro for this, but we will use the fancier [`def-actor`](https://github.com/open-goal/jak-project/blob/48cb9bb7871c8d566176ededec42327c3dfd6086/goal_src/jak1/engine/data/art-h.gc#L314) macro that will cut out a lot of the boilerplate.

The `defskelgroup` macro usually requires at minimum:
...
The `def-actor` macro, on the other hand, will cut most of this boilerplate out, so at minimum, you only need to give it the actor name and a bounds sphere:

```opengoal
;; the def-actor macro automatically defines the default "art-elts" for the joint geometry, mesh group and idle animation for us
(def-actor test-actor :bounds (0 0 0 5))
```

Optionally, if your model has some actual animations and/or joints, you can also define those with the `:art` and `:joints` keys as lists to easily reference them later in code.

```opengoal
(def-actor test-actor
  :bounds (0 0 0 5)
  :art (idle-ja sleep-ja wake-ja moving-ja) ;; will define constants for each animation, e.g. "test-actor-wake-ja"
  :joints (align prejoint main lower-body upper-body chest head) ;; will define joints for the actor that can be used by the "joint-node" macro
  )
```

By default, the initial animation will be called `test-actor-idle-ja` and use the first animation defined in the art group. You can override this with the `:idle` key. This is the animation that the actor will default to when being spawned as a `viewer-process` or when you don't manually change it to play a different one.

```opengoal
(def-actor test-actor
  :bounds (0 0 0 5)
  :art (idle-ja sleep-ja wake-ja moving-ja)
  :joints (align prejoint main lower-body upper-body chest head)
  :idle sleep-ja ;; set the "default" animation to sleep-ja instead of idle-ja
  )
```
