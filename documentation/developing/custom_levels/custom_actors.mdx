...
While still in very early stages, the "Build Actor" tool allows you to take a mesh created in Blender and import it into the game to use for your own custom actors in custom levels.
...
You cannot import animations for custom actors. Every actor just has a default `idle` animation that does nothing.

The animation system for Jak is extremely complex and the way the animation data is stored is still not very well understood. Due to this, it will be a while before custom animations become a thing.

### Joints/Bones

Currently, every model will have a single default `main` bone, located at the model root.
...
- Working OpenGOAL install with tooling version >=`0.2.13`
- [Blender 4.0](https://download.blender.org/release/Blender4.0/) (needs to be this exact version due to GLTF exporter changes in later versions that break compatibility with OpenGOAL)
...
For the `art-joint-geo`, we just define a single `main` joint with a default bind pose for now. In the future, we would ideally be able to extract the armature data from the model file so each model can have a proper bone structure.

For the `art-joint-anim`, we just steal the animation data from another vanilla art group that just has a default idle animation with nothing going on. The format for animation data is still kind of a mystery, so we have to make do with no custom animations for the time being.

### `skeleton-group`

The `skeleton-group` is technically also an `art-group`, but instead of being stored as a binary GOAL object file, it's actually defined in code. It also serves a slightly different purpose: It stores settings related to the actor in question, such as the name of the art group it needs, the default animation and joint geometry, shadow and LOD settings.

### GOAL Code

Every actor needs to be defined as its own type that inherits from `process-drawable` and have some logic set up in order to be able to be spawned and interacted with. If there is no code for the actor, its entity will spawn a `viewer-process` instead, which will, as the name suggests, just display the model and do the default idle animation, and it cannot be interacted with in any way.
...
### `custom_assets/jak1/models/test-actor.glb`
...
Add `"test-actor"` to the `custom_models` list.
...
Right now, the art group that is generated for the object will always have the joint geometry at index 0, the mesh group at index 1 and the default animation at index 2.
...
The `def-art-elt` definitions I added are not strictly required, but it just makes it look neater.
...
- The index of the bone we attach to (`(-> cgroup transform-index)`)
...
(defmethod init-from-entity! ((this test-actor) (e entity-actor))
  ...
  (process-drawable-from-entity! this e) ;; read position and rotation from entity
